#!/bin/bash

# ================================================
# Умный скрипт для запуска проекта в гибридном режиме
# ================================================
# Фронт и бэк локально, база и Redis в Docker
# Автоматически решает проблемы с портами, Docker, Prisma

set -e

echo "🚀 Запуск Fin-U-CH в гибридном режиме..."
echo "📦 База данных и Redis в Docker"
echo "💻 Фронт и бэк локально"
echo ""

# Функция для освобождения портов
free_ports() {
    echo "🔧 Проверяем и освобождаем порты..."
    
    # Список портов для проверки
    PORTS=(4000 5173 5432 6379)
    
    for port in "${PORTS[@]}"; do
        # Находим процессы на порту
        PIDS=$(lsof -ti:$port 2>/dev/null || true)
        if [ ! -z "$PIDS" ]; then
            echo "⚠️  Порт $port занят, освобождаем..."
            echo "$PIDS" | xargs kill -9 2>/dev/null || true
            sleep 1
        fi
    done
    
    echo "✅ Порты освобождены"
}

# Функция для проверки и запуска Docker
ensure_docker() {
    echo "🐳 Проверяем Docker..."
    
    # Проверяем наличие Docker
    if ! command -v docker &> /dev/null; then
        echo "❌ Docker не установлен. Установите Docker и попробуйте снова."
        exit 1
    fi
    
    # Проверяем, запущен ли Docker daemon
    if ! docker info &> /dev/null; then
        echo "⚠️  Docker не запущен, запускаем..."
        open -a Docker
        echo "⏳ Ждем запуска Docker (30 сек)..."
        sleep 30
        
        # Проверяем еще раз
        if ! docker info &> /dev/null; then
            echo "❌ Не удалось запустить Docker. Запустите Docker Desktop вручную."
            exit 1
        fi
    fi
    
    echo "✅ Docker готов"
}

# Проверяем наличие pnpm
if ! command -v pnpm &> /dev/null; then
    echo "❌ pnpm не установлен. Установите pnpm и попробуйте снова."
    exit 1
fi

# Выполняем предварительные проверки
free_ports
ensure_docker

# Проверяем наличие .env файла
if [ ! -f .env ]; then
    echo "❌ Файл .env не найден!"
    echo "📝 Создайте .env файл вручную:"
    echo "   pnpm env:setup"
    echo "   или"
    echo "   cp env.example .env"
    exit 1
fi

# Устанавливаем зависимости если нужно
if [ ! -d "node_modules" ]; then
    echo "📦 Устанавливаем зависимости..."
    pnpm install
fi

# Генерируем Prisma клиент
echo "🔧 Генерируем Prisma клиент..."
pnpm prisma:generate

# Запускаем Docker контейнеры
echo "🐳 Запускаем Docker контейнеры (PostgreSQL + Redis)..."
pnpm docker:hybrid:up

# Ждем пока база данных будет готова
echo "⏳ Ждем готовности базы данных..."
sleep 10

# Проверяем подключение к базе данных
echo "🔍 Проверяем подключение к базе данных..."
for i in {1..10}; do
    if pnpm --filter api prisma db push --accept-data-loss &>/dev/null; then
        echo "✅ База данных готова"
        break
    else
        echo "⏳ Попытка $i/10 - ждем базу данных..."
        sleep 3
    fi
done

# Запускаем миграции
echo "🗄️ Запускаем миграции базы данных..."
pnpm --filter api prisma:migrate

# Создаем демо пользователя (игнорируем ошибку если уже существует)
echo "👤 Создаем демо пользователя..."
pnpm demo:create || echo "ℹ️  Демо пользователь уже существует"

echo ""
echo "✅ Готово! Проект запущен в гибридном режиме:"
echo "   🌐 Frontend: http://localhost:5173"
echo "   🔧 API: http://localhost:4000"
echo "   🗄️ PostgreSQL: localhost:5432"
echo "   📦 Redis: localhost:6379"
echo ""
echo "Для остановки используйте: pnpm dev:hybrid:stop"
echo ""

# Запускаем фронт и бэк локально
echo "🚀 Запускаем фронт и бэк локально..."

# Запускаем API в фоне
pnpm --filter api dev &
API_PID=$!

# Ждем запуска API
echo "⏳ Ждем запуска API..."
sleep 5

# Проверяем, что API запустился
if ! kill -0 $API_PID 2>/dev/null; then
    echo "❌ API не запустился"
    exit 1
fi

# Запускаем Web в фоне
pnpm --filter web dev &
WEB_PID=$!

# Ждем запуска Web
echo "⏳ Ждем запуска Web..."
sleep 3

# Проверяем, что Web запустился
if ! kill -0 $WEB_PID 2>/dev/null; then
    echo "❌ Web не запустился"
    kill $API_PID 2>/dev/null || true
    exit 1
fi

# Функция для проверки авторизации демо-пользователя
check_demo_auth() {
    echo "🔍 Проверяем авторизацию демо-пользователя..."
    
    # Ждем немного, чтобы API точно запустился
    sleep 3
    
    # Пробуем авторизоваться
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST http://localhost:4000/api/auth/login \
        -H "Content-Type: application/json" \
        -d '{"email":"demo@example.com","password":"demo123"}' 2>&1 || echo -e "\n000")
    
    HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
    BODY=$(echo "$RESPONSE" | sed '$d')
    
    # Убираем лишние пробелы и переносы строк из HTTP_CODE
    HTTP_CODE=$(echo "$HTTP_CODE" | tr -d '[:space:]')
    
    if [ "$HTTP_CODE" = "500" ]; then
        echo "⚠️  Обнаружена ошибка 500 при авторизации демо-пользователя"
        echo "🔄 Обновляем миграции базы данных..."
        
        # Сохраняем текущую директорию
        ORIGINAL_DIR=$(pwd)
        
        # Обновляем схему базы данных
        if [ -d "apps/api" ]; then
            cd apps/api
            if npx prisma db push --accept-data-loss 2>/dev/null; then
                echo "✅ Схема базы данных обновлена"
            else
                echo "⚠️  Не удалось обновить схему через db push, пробуем migrate deploy..."
                npx prisma migrate deploy 2>/dev/null || echo "⚠️  Не удалось применить миграции"
            fi
            cd "$ORIGINAL_DIR"
        else
            echo "⚠️  Директория apps/api не найдена"
        fi
        
        # Генерируем Prisma клиент заново
        echo "🔧 Генерируем Prisma клиент..."
        pnpm prisma:generate || echo "⚠️  Не удалось сгенерировать Prisma клиент"
        
        echo "✅ Миграции обновлены"
        echo "⏳ Ждем перезапуска API (nodemon должен автоматически перезапустить)..."
        sleep 5
        
        # Пробуем еще раз
        echo "🔍 Повторная проверка авторизации..."
        RESPONSE=$(curl -s -w "\n%{http_code}" -X POST http://localhost:4000/api/auth/login \
            -H "Content-Type: application/json" \
            -d '{"email":"demo@example.com","password":"demo123"}' 2>&1 || echo -e "\n000")
        
        HTTP_CODE=$(echo "$RESPONSE" | tail -n1 | tr -d '[:space:]')
        
        if [ "$HTTP_CODE" = "200" ]; then
            echo "✅ Авторизация работает после обновления миграций"
        else
            echo "⚠️  Авторизация все еще не работает (HTTP $HTTP_CODE)"
            echo "   Возможно, нужно перезапустить API вручную"
        fi
    elif [ "$HTTP_CODE" = "200" ]; then
        echo "✅ Авторизация демо-пользователя работает"
    else
        echo "ℹ️  Авторизация вернула код $HTTP_CODE (это нормально, если демо-пользователь не создан)"
    fi
}

# Проверяем авторизацию демо-пользователя
# Отключаем set -e для этой функции, чтобы ошибки не прерывали скрипт
set +e
check_demo_auth
set -e

# Функция для корректного завершения
cleanup() {
    echo ""
    echo "🛑 Останавливаем сервисы..."
    
    # Останавливаем процессы
    kill $API_PID $WEB_PID 2>/dev/null || true
    
    # Ждем завершения процессов
    wait $API_PID $WEB_PID 2>/dev/null || true
    
    # Останавливаем Docker контейнеры
    echo "🐳 Останавливаем Docker контейнеры..."
    pnpm docker:hybrid:down
    
    echo "✅ Все сервисы остановлены"
    exit 0
}

# Обработчик сигналов для корректного завершения
trap cleanup SIGINT SIGTERM

echo ""
echo "🎉 Проект успешно запущен!"
echo "   🌐 Frontend: http://localhost:5173"
echo "   🔧 API: http://localhost:4000"
echo "   🗄️ PostgreSQL: localhost:5432"
echo "   📦 Redis: localhost:6379"
echo ""
echo "Для остановки нажмите Ctrl+C"
echo ""

# Ждем завершения
wait
