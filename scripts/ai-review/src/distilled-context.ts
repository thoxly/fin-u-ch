...
async function getFileCachePath(): Promise<string> {
  const projectRoot = CONFIG.review.projectRoot;
  return path.join(projectRoot, CACHE_FILE_NAME);
}

async function getFileCache(): Promise<string | null> {
  try {
    ...
    if (age > CACHE_TTL_MS) {
      console.log(
        `File cache expired (age: ${Math.round(age / (1000 * 60 * 60 * 24))} days), will regenerate`
      );
      return null;
    }
    ...
    console.log(
      `Using distilled context from file cache (age: ${Math.round(age / (1000 * 60 * 60 * 24))} days)`
    );
    return content;
  } catch (err: any) {
    if (err.code === 'ENOENT') {
...
async function setFileCache(content: string): Promise<void> {
  try {
    const cachePath = await getFileCachePath();
