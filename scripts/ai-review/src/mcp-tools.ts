...
async function findSimilarFiles(targetPath: string): Promise<string[]> {
  ...
  try {
    ...
    const scored = Array.from(allResults).map((file) => {
      ...
      if (path.basename(file) === basename) score += 100;

      // Same directory structure
      const fileDirname = path.dirname(file);
      if (fileDirname === dirname) score += 50;
      else if (fileDirname.includes(dirname) || dirname.includes(fileDirname)) {
        score += 25;
      }

      // Same extension
      if (path.extname(file) === ext) score += 10;

      // Similar name
      const fileNameWithoutExt = path.parse(file).name;
      if (fileNameWithoutExt.includes(nameWithoutExt)) score += 20;
      if (nameWithoutExt.includes(fileNameWithoutExt)) score += 15;
