      ...
      <div className="max-h-[300px] overflow-y-auto">
        <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">
          ...
          <div className="space-y-1">
            {(showAll ? operations : operations.slice(0, maxVisible)).map((op) => (
              <div key={op.id} className="text-xs p-2 bg-gray-50 dark:bg-gray-700 rounded">
                <div className="flex justify-between items-start gap-2 mb-1">
                  <span className={`font-medium ${
                    op.type === 'income' ? 'text-green-600 dark:text-green-400' : 
                    op.type === 'expense' ? 'text-red-600 dark:text-red-400' : 
...
export const IncomeExpenseChart: React.FC<IncomeExpenseChartProps> = ({ data, className = '' }) => {
        ...
        <div className="h-80 relative">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={filteredData || []} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <XAxis 
                ...
                textAnchor={filteredData && filteredData.length > 8 ? 'end' : 'middle'}
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center text-gray-500 dark:text-gray-400 bg-white/80 dark:bg-gray-800/80 px-4 py-2 rounded-lg">
              ...
              <div className="text-sm font-medium">Нет данных за выбранный период</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <LineChart data={filteredData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line 
              ...
              dot={(props: any) => {
                ...
                const hasIncome = operations.some((op: Operation) => op.type === 'income');
                if (!hasIncome) return null;
                return <circle cx={cx} cy={cy} r={3} fill="#10b981" strokeWidth={1.5} stroke="#10b981" />;
              }}
              name="Доходы"
            />
            <Line 
              ...
              dot={(props: any) => {
                ...
                const operations = payload?.operations || [];
                const hasExpense = operations.some((op: Operation) => op.type === 'expense');
                if (!hasExpense) return null;
                return <circle cx={cx} cy={cy} r={3} fill="#ef4444" strokeWidth={1.5} stroke="#ef4444" />;
              }}
              name="Расходы"
            />
            <Line 
