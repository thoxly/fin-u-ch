import { SelectHTMLAttributes, forwardRef, Fragment, useState, useRef, useEffect, useCallback } from 'react';
...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  (
    ...
    const findScrollContainer = useCallback((element: HTMLElement | null): { top: number; bottom: number; left: number; right: number } | null => {
      ...
      while (current && current !== document.body) {
        ...
        if (isModal || overflow.includes('auto') || overflow.includes('scroll') || overflow.includes('hidden')) {
      ...
      if (isOpen) {
        ...
        const handleScroll = (e: Event) => {
          ...
          if (target && buttonRef.current) {
            ...
            if (isRelatedScroll || target === document.body || target === document.documentElement) {
    ...
    useEffect(() => {
      ...
      const checkOpen = () => {
        if (buttonRef.current) {
          const expanded = buttonRef.current.getAttribute('aria-expanded') === 'true';
          if (expanded !== isOpen) {
            setIsOpen(expanded);
            if (expanded) {
      ...
      >
        ...
        >
          {options.map((option, index) => {
            ...
            const showDivider = isCreateOption && prevOption && String(prevOption.value) !== '__create__';
