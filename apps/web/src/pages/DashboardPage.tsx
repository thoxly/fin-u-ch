...
import { useGetDashboardQuery, useGetCumulativeCashFlowQuery } from '../store/api/reportsApi';
...
export const DashboardPage = () => {
  ...
  const { data: cumulativeData, isLoading: cumulativeLoading } = useGetCumulativeCashFlowQuery({
    periodFrom: periodFilters.range.from,
    periodTo: periodFilters.range.to,
    mode: 'both',
  ...
  const { data: operations = [], isLoading: operationsLoading } = useGetOperationsQuery({
  ...
  const accountBalancesData = dashboardData?.accountBalancesSeries.map(series => {
    const result: { date: string; label: string; [accountName: string]: string | number } = {
      date: series.date,
      label: series.label,
    };
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-400 dark:text-gray-500">
                      за период {periodFilters.range.from} - {periodFilters.range.to}
                    </div>
                  </div>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="w-8 h-8 bg-red-100 dark:bg-red-900 rounded-full flex items-center justify-center">
                    <svg className="w-4 h-4 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 17h8m0 0V9m0 8l-8-8-4 4" />
                    </svg>
                  </div>
                </div>
              </Card>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className={`text-2xl font-bold mb-1 ${
                      ...
                      {dashboardData.summary.netProfit >= 0 ? '+' : ''}{formatMoney(dashboardData.summary.netProfit)}
                    </div>
                    <div className="text-xs text-gray-400 dark:text-gray-500">
                      за период {periodFilters.range.from} - {periodFilters.range.to}
                    </div>
