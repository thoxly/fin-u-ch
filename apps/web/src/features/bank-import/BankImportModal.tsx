...
import { Upload, Minimize2, Maximize2, X, Info, ChevronDown, ChevronUp } from 'lucide-react';
...
export const BankImportModal = ({ isOpen, onClose }: BankImportModalProps) => {
  ...
  useEffect(() => {
    ...
    } else if (!isOpen && !isMinimized && (collapsedHistory || collapsedMapping)) {
      // Сохраняем состояние даже если модальное окно закрыто, но есть свернутые секции
      const state: StoredState = {
        sessionId: null,
        minimized: false,
        timestamp: Date.now(),
  ...
  }, [isOpen, sessionId, isMinimized, collapsedHistory, collapsedMapping, activeTab, viewingSessionId]);
        ...
        {activeTab === 'upload' ? (
          <div className="space-y-4">
            ...
            <div className={`rounded-lg p-4 ${
              ...
              >
                <div className="flex items-center gap-2">
                  <Info className={!company?.inn ? 'text-orange-600 dark:text-orange-400' : 'text-blue-600 dark:text-blue-400'} size={20} />
                ...
                {showInnInfo ? (
                  <ChevronUp className={!company?.inn ? 'text-orange-600 dark:text-orange-400' : 'text-blue-600 dark:text-blue-400'} size={20} />
                ) : (
                  <ChevronDown className={!company?.inn ? 'text-orange-600 dark:text-orange-400' : 'text-blue-600 dark:text-blue-400'} size={20} />
                )}
              </button>
              {showInnInfo && (
                <div className={`mt-3 text-sm space-y-2 ${
                  ...
                  <ul className="list-disc list-inside space-y-1 ml-2">
                    <li>
                      Если в платежном поручении в строке <strong>Плательщик</strong> указан ИНН вашей компании, 
                      система автоматически определит операцию как <strong>Списание</strong>.
                    </li>
                    <li>
                      Если в строке <strong>Получатель</strong> указан ИНН вашей компании, 
                      система автоматически определит операцию как <strong>Поступление</strong>.
                    </li>
                  </ul>
                  {!company?.inn && (
                    <p className="mt-2 font-medium text-orange-700 dark:text-orange-300">
                      💡 Перейдите в настройки профиля, чтобы добавить ИНН и упростить импорт выписок.
                    </p>
                  )}
                </div>
              )}
