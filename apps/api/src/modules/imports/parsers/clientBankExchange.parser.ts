...
export function parseClientBankExchange(content: Buffer | string): ParsedFile {
  ...
  if (Buffer.isBuffer(content)) {
    ...
    try {
      ...
      const withoutBom = trimmed.length > 0 && trimmed.charCodeAt(0) === 0xfeff ? trimmed.slice(1) : trimmed;
      ...
      const hasHeader = firstLines.some(line => line.includes('1CClientBankExchange'));
      ...
      if (hasHeader) {
        ...
        const utf8WithoutBom = utf8Trimmed.length > 0 && utf8Trimmed.charCodeAt(0) === 0xfeff ? utf8Trimmed.slice(1) : utf8Trimmed;
        ...
        const utf8HasHeader = utf8FirstLines.some(line => line.includes('1CClientBankExchange'));
        ...
        if (utf8HasHeader) {
          ...
          text = win1251Text;
          logger.debug('Parser: Using Windows-1251 encoding (default for 1C files)');
        }
      }
  ...
  if (!foundHeader) {
    ...
    const firstLines = lines.slice(0, 3).map(l => l.substring(0, 50)).join(' | ');
    const debugBytes = Buffer.isBuffer(content) 
      ? Array.from(content.slice(0, 30)).map(b => b.toString(16).padStart(2, '0')).join(' ')
      : 'not a buffer';
    throw new AppError(
  ...
  for (let i = 0; i < fileLines.length; i++) {
    ...
    if (normalizedLine.includes('секциядокумент')) {
      ...
      if (equalIndex > 0 || equalIndex === -1) {
        ...
        const docType = equalIndex > 0 ? line.substring(equalIndex + 1).trim() : '';
        documentsStarted++;
        ...
        if (isPaymentOrder) {
          ...
          logger.info(`Parser: Skipping document type "${docType}" at line ${i + 1}`, {
            originalLine: line.substring(0, 100)
          });
          documentsSkipped++;
          inDocument = false;
      ...
      if (equalIndex > 0) {
        ...
        switch (normalizedKey) {
          ...
          case 'плательщикинн':
            ...
            break;
          case 'плательщиксчет':
            currentDocument.payerAccount = validateAccountNumber(value) || undefined;
            break;
          case 'получатель':
  ...
  if (documents.length === 0) {
    ...
    if (documentsStarted > 0) {
      ...
      errorMessage += ' Please check that the file contains sections starting with "СекцияДокумент=Платежное поручение".';
    }
...
function parseDate(dateStr: string): Date {
  ...
  if (parts.length !== 3) {
    throw new AppError(`Invalid date format: ${dateStr}. Expected DD.MM.YYYY`, 400);
  }
