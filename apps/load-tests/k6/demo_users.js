...
function createMultipleOperations(headers, accounts, articles, counterparties, count = 5) {
  for (let i = 0; i < count; i++) {
    ...
    if (operationType === 'transfer') {
      // Для transfer операций нужны sourceAccountId и targetAccountId
      if (Array.isArray(accounts) && accounts.length >= 2) {
        operationPayload.sourceAccountId = accounts[0].id;
        operationPayload.targetAccountId = accounts[1].id;
        // НЕ устанавливаем accountId для transfer операций
      } else {
        // Пропускаем transfer, если недостаточно счетов
        console.log('Skipping transfer operation: need at least 2 accounts');
        continue;
      }
    } else {
      // Для income/expense операций нужны accountId и articleId
      if (Array.isArray(accounts) && accounts.length > 0) {
        operationPayload.accountId = accounts[0].id;
      }

      // Установка ID статьи только для income/expense операций
    ...
    // Установка ID контрагента (опционально для всех типов операций)
